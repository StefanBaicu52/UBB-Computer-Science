...
class CityManagement:
    ...
    def __read_input(self):
        ...
        try:
            ...
            if len(data) < 9:
                raise ValueError(f"Fișierul 'year1.txt' nu conține suficiente date! Date găsite: {data}")

            self.__year = data[0]
            self.__starving_people = data[1]
            self.__new_people = data[2]
            self.__population = data[3]
            self.__acres_of_land = data[4]
            self.__harvest = data[5]
            self.__rat_units = data[6]
            self.__land_price = data[7]
            self.__stocks = data[8]

        except FileNotFoundError:
            print("Eroare: Fișierul 'year1.txt' nu a fost găsit!")
           # exit(1) NUJ DACA TRABA
