...
bool Service::userAdoptService(const std::string& photograph) {
    ...
    if (it != dogs.end()) {
        it->set_adopted(true);                  // ✅ corect: marchează ca adoptat mai întâi
        this->adoptionlist->addDog(*it);        // ✅ apoi îl salvează în fișier
        return true;
    }
    return false;
}
...
// Opens the adoption list file (CSV or HTML)
